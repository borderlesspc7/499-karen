...
export function useThemeClasses() {
  ...
  return useMemo(
    () => ({
      ...
      // Fundos de card SEMPRE opacos — bg com /opacity + elevation no Android
      // cria a "caixa cinza com retângulo branco" (shadow sem backgroundColor sólido).
      card: isDark
        ? 'rounded-card border border-white/10 bg-summus-900'
        : 'rounded-card border border-premiumBorder bg-white',
      cardLg: isDark
        ? 'rounded-3xl border border-white/10 bg-summus-900'
        : 'rounded-3xl border border-premiumBorder bg-white',
      cardSm: isDark
        ? 'rounded-2xl border border-white/10 bg-summus-900'
        : 'rounded-2xl border border-premiumBorder bg-white',
      panel: isDark
      ...
      surfaceMuted: isDark ? 'bg-summus-800' : 'bg-slate-50',
      surfaceInset: isDark
        ? 'bg-summus-800 border-white/10'
        : 'bg-surface border-premiumBorder/60',
      ...
      filterInactive: isDark
        ? 'border border-white/10 bg-summus-900'
      ...
      emptyState: isDark
        ? 'border border-dashed border-white/15 bg-summus-900'
        : 'border border-dashed border-slate-200 bg-white',
      progressTrack: isDark ? 'bg-white/10' : 'bg-slate-100',
      incomingBubble: isDark
        ? 'border border-white/10 bg-summus-800'
        : 'border border-slate-200 bg-white',
      iconButton: isDark
        ? 'border border-white/10 bg-summus-800 active:bg-white/10'
      ...
      footerBar: isDark
        ? 'border-t border-white/10 bg-navy'
        : 'border-t border-premiumBorder bg-white',
      tabInactive: isDark
        ? 'border border-white/10 bg-summus-900'
        : 'border border-premiumBorder bg-white',
      tabInactiveText: isDark ? 'text-white/60' : 'text-slate-500',
      input: isDark
        ? 'rounded-2xl border border-white/10 bg-summus-900 px-4 py-4 text-base leading-6 text-white'
        : 'rounded-card border border-premiumBorder bg-white px-4 py-4 text-base leading-6 text-navy',
      inputInline: isDark
        ? 'rounded-2xl border border-white/10 bg-summus-800 px-4'
      ...
      glassCard: isDark
        ? 'overflow-hidden rounded-3xl border border-white/10 bg-summus-900'
        : 'overflow-hidden rounded-3xl border border-premiumBorder bg-white',
      heroCard: isDark
        ? 'overflow-hidden rounded-3xl border border-white/10 bg-summus-900'
        : 'overflow-hidden rounded-3xl border border-premiumBorder bg-white',
