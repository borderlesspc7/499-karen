...
exports.stripeWebhook = exports.createBillingPortalSession = exports.confirmMockCheckout = exports.createCheckoutSession = void 0;
/**
...
async function writeUserSubscription(userId, subscription) {
...
function parseInterval(value) {
    ...
    throw new https_1.HttpsError('invalid-argument', 'Intervalo de cobrança inválido.');
}
/**
...
exports.createCheckoutSession = (0, https_1.onCall)({ secrets: [stripeSecretKey], cors: true }, async (request) => {
    ...
    const cancelUrl = typeof request.data?.cancelUrl === 'string' ? request.data.cancelUrl : 'https://example.com/cancel';
    const priceId = resolvePriceId(planId, billingInterval);
    const sessionId = `cs_mock_${(0, crypto_1.randomBytes)(8).toString('hex')}`;
...
exports.confirmMockCheckout = (0, https_1.onCall)({ cors: true }, async (request) => {
    ...
    if (!sessionId.startsWith('cs_mock_') && !sessionId.startsWith('cs_test_')) {
        throw new https_1.HttpsError('failed-precondition', 'confirmMockCheckout só aceita sessões mock. Use o webhook Stripe em produção.');
    }
    ...
    if (sessionDoc.exists) {
        ...
        if (data?.status === 'complete') {
            ...
            const subscription = existing.data()?.subscription;
            if (subscription) {
                return { ok: true, subscription };
            }
        }
    }
    const eventId = `evt_mock_${(0, crypto_1.createHash)('sha256').update(sessionId).digest('hex').slice(0, 24)}`;
    ...
    const subscription = buildActiveSubscription({
        ...
        stripeSubscriptionId: `sub_mock_${(0, crypto_1.randomBytes)(6).toString('hex')}`,
    });
 ...
 */
exports.createBillingPortalSession = (0, https_1.onCall)({ secrets: [stripeSecretKey], cors: true }, async (request) => {
    requireAuth(request);
    if (!isStripeLiveMode()) {
        return { url: null, mode: 'mock' };
    }
    // const stripe = new Stripe(stripeSecretKey.value())
 ...
 */
exports.stripeWebhook = (0, https_1.onRequest)({ secrets: [stripeSecretKey, stripeWebhookSecret], cors: false }, async (req, res) => {
    if (req.method !== 'POST') {
