...
export function ActiveCampaignCard({ isJustActivated = false }: ActiveCampaignCardProps) {
  ...
  const campaignMetrics = [
    { id: 'views', label: t('home.views'), value: '14.5k' },
    { id: 'leads', label: t('home.leadsGenerated'), value: '42' },
    { id: 'cpl', label: t('home.costPerLead'), value: 'R$ 8,50' },
  ] as const
        ...
        <View className="flex-row items-start justify-between gap-3">
          <View className="flex-1 gap-2">
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-xs font-bold uppercase tracking-wider text-gold">
                {t('home.activeCampaignBadge')}
              </Text>
            </View>
            <Text className="text-2xl font-bold tracking-tight text-white">{t('home.newService')}</Text>
            <Text className="text-sm leading-5 text-white/55">
              {isJustActivated ? t('home.campaignJustLive') : t('home.campaignOmnichannelResults')}
            </Text>
          </View>
          ...
          <View className="flex-row items-center gap-1.5 rounded-full border border-emerald/20 bg-emerald/10 px-3 py-1.5">
            ...
            <Text className="text-[11px] font-bold text-emerald">
              {isJustActivated ? t('home.activatedNow') : t('home.live')}
            </Text>
          </View>
        </View>
