...
export function InboxAiSummary({
    ...
    return (
      <View className="flex-row items-center gap-1.5">
        ...
        <Text className={['flex-1 text-xs', tc.textMuted].join(' ')} numberOfLines={1}>
          {t('inbox.aiSummaryCompact', { summary })}
        </Text>
        {estimatedValue ? (
  ...
  return (
    <View
      className={[
        'flex-row items-start gap-2 rounded-xl border border-gold/15 bg-gold/5 px-3 py-2',
        tc.cardSm,
      ].join(' ')}
    >
      <View
      ...
      <View className="min-w-0 flex-1">
        ...
        {estimatedValue ? (
          <Text className="mt-1 text-xs font-semibold text-emerald">
            {t('inbox.potential', { value: formatCurrencyBrlCompact(estimatedValue) })}
          </Text>
        ) : null}
      </View>
    </View>
